 ...
 *
 * Return: 1 if the strings can be considered identical if not 0.
 */
...
int bandersnatch(char *s1, char *s2)
{
	 ...
	 * if chars are equal, return the returned value of wildcmp()
...
{
	/**
	 * if the current char past the *
	 * increment s2 by 1
	 * else return te address of the
	 * first char past all *
	 */
	if (*s2 == '*')
		return (move(s2 + 1));
